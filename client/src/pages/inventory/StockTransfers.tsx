...
export default function StockTransfers() {
  ...
  const resetForm = () => {
    setForm({ fromWarehouseId: "", toWarehouseId: "", date: new Date().toISOString().split("T")[0], notes: "" });
  ...
  const statusLabel = (s: string) => ({ draft: "مسودة", confirmed: "مؤكد", cancelled: "ملغي" }[s] || s);
  const statusColor = (s: string) => ({ draft: "outline", confirmed: "default", cancelled: "destructive" }[s] || "outline") as any;

  return (
    <ERPLayout title="التحويلات بين المخازن">
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-slate-50">
                <TableHead className="text-right text-xs font-semibold text-slate-600">رقم التحويل</TableHead>
                <TableHead className="text-right text-xs font-semibold text-slate-600">من مخزن</TableHead>
                <TableHead className="text-right text-xs font-semibold text-slate-600">إلى مخزن</TableHead>
      ...
      <Dialog open={open} onOpenChange={v => { setOpen(v); if (!v) resetForm(); }}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto" dir="rtl">
          <DialogHeader><DialogTitle>تحويل مخزني جديد</DialogTitle></DialogHeader>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button size="sm" className="bg-purple-600 hover:bg-purple-700 text-white" onClick={handleSubmit} disabled={createMut.isPending}>حفظ التحويل</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </ERPLayout>
