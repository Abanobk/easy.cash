      ...
      <div ref={printRef} className="bg-white rounded-xl border border-gray-200 shadow-sm p-8">
        ...
        <div className="flex justify-between items-start mb-6 pb-4 border-b-2 border-blue-600">
          <div className="flex items-start gap-3">
            {company?.logo && (
              <img src={company.logo} alt="شعار" className="w-14 h-14 object-contain rounded-lg border border-gray-100" />
            )}
            <div>
            <div className="text-2xl font-bold text-blue-700">{company?.name || "Easy Cash"}</div>
            {company?.address && <div className="text-xs text-gray-500 mt-0.5">{company.address}</div>}
            {company?.phone && <div className="text-xs text-gray-500">هاتف: {company.phone}</div>}
            {company?.taxNumber && <div className="text-xs text-gray-500">الرقم الضريبي: {company.taxNumber}</div>}
            {!company?.address && <div className="text-sm text-gray-500 mt-1">نظام المحاسبة والإدارة المتكامل</div>}
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
            <div className="text-xs text-blue-500 mb-1">العميل</div>
            <div className="font-semibold text-gray-800">{inv.customerName || `عميل #${inv.customerId}`}</div>
          </div>
