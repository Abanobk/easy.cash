...
export default function SalesInvoices() {
  ...
  const { data, isLoading, refetch } = trpc.sales.invoices.list.useQuery({ page, limit: 20 });
  ...
  const addItem = () => {
    setInvoiceItems(prev => [...prev, { itemId: 0, itemName: "", itemUnit: "", quantity: "1", price: "0", discount: "0", tax: "0", total: "0" }]);
  };
  ...
  const updateItem = (idx: number, field: keyof InvoiceItem, value: string) => {
    setInvoiceItems(prev => {
      ...
      updated[idx].total = (subtotal * (1 + t / 100)).toFixed(2);
      return updated;
    });
  };
  ...
  if (showForm) {
    return (
      <ERPLayout title="فاتورة مبيعات جديدة">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    ...
                    {invoiceItems.length === 0 && (
                      <tr><td colSpan={7} className="py-8 text-center text-slate-400 text-xs">اضغط "إضافة صنف" لإضافة أصناف للفاتورة</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
                  ...
                  <div className="flex justify-end">
                    <div className="w-64 space-y-2">
                      <div className="flex justify-between text-sm text-slate-600">
                        ...
                        <span className="font-medium">{subtotal.toLocaleString("ar-EG", { minimumFractionDigits: 2 })} ج.م</span>
                      </div>
                      <div className="flex justify-between text-sm text-slate-600">
                        <span>الضريبة:</span>
                        <span className="font-medium">{taxTotal.toLocaleString("ar-EG", { minimumFractionDigits: 2 })} ج.م</span>
                      </div>
                      <div className="flex justify-between text-base font-bold text-slate-800 border-t border-slate-200 pt-2">
                        <span>الإجمالي:</span>
                        <span className="text-blue-600">{total.toLocaleString("ar-EG", { minimumFractionDigits: 2 })} ج.م</span>
                      </div>
                    </div>
                  </div>
                </div>
  ...
  return (
    <ERPLayout title="فواتير المبيعات">
      <DataTable
