/**
 * Capture Mega wave-6 profits + vendor credits aging PDFs.
...
const OUT = "artifacts/mega-wave6-profits/pdf";
...
const REPORTS = [
  { key: "itemsprofits", featureKey: "accountingreports-itemsprofits", path: "/AccountingReports/ItemsProfits.aspx" },
  { key: "customersprofits", featureKey: "accountingreports-customersprofits", path: "/AccountingReports/CustomersProfits.aspx" },
  { key: "invoiceprofits", featureKey: "accountingreports-invoiceprofits", path: "/AccountingReports/InvoiceProfits.aspx" },
  { key: "creditsages", featureKey: "accountingreports-creditsages", path: "/AccountingReports/CreditsAges.aspx" },
  { key: "creditsagesbyyear", featureKey: "accountingreports-creditsagesbyyear", path: "/AccountingReports/CreditsAgesByYear.aspx" },
  { key: "creditsagesbyhalfyear", featureKey: "accountingreports-creditsagesbyhalfyear", path: "/AccountingReports/CreditsAgesByHalfYear.aspx" },
];
...
async function captureOne(page, report) {
  ...
  if (!frame) return { key: report.key, featureKey: report.featureKey, path: report.path, error: "no-MainIframe" };

  // Detect auth / wrong page early
  ...
  if (gate.authDenied || gate.startScreen || (!gate.hasShow && !gate.hasDateFrom)) {
  ...
  const prep = await frame.evaluate(() => {
    ...
    // Aging reports often use as-of / single date
    const okAsOf =
    ...
    return {
      ...
      nearbyFilterHints: labels.filter((t) =>
        /فرع|تاريخ|عميل|مورد|صنف|مخزن|مندوب|منطقة|فئة|ترتيب|حالة|اخفاء|عرض|مسلسل|باركود/i.test(t)
      ),
    };
  });
...
(async () => {
  ...
  for (const report of list) {
    ...
    try {
      ...
      results.push({ key: report.key, featureKey: report.featureKey, path: report.path, error: String(e) });
    }
  }

  fs.writeFileSync(path.join(OUT, "wave6-pdf-capture.json"), JSON.stringify({ at: new Date().toISOString(), results }, null, 2));
  console.log(
    "SUMMARY",
    JSON.stringify(
      results.map((r) => ({
