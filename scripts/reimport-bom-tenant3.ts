/**
 * إعادة استيراد تركيبات المنتجات (BOM) لمستأجر tenant 3 من ملف
 * "تقدير الكميات بالمكونات" الفعلي — بنفس منطق شاشة "استيراد تقارير Mega":
 ...
 *   docker run --rm --network easy_cash_accounting_default \
 *     -v "$PWD":/app -w /app \
 *     -v "$HOME/Downloads":/data \
 *     -e DATABASE_URL='mysql://easy_cash:<pw>@mysql:3306/easy_cash' \
 *     -e JWT_SECRET=x -e ENCRYPTION_KEY=x -e ALLOW_WEAK_SECRETS=1 \
 *     node:20-slim node node_modules/tsx/dist/cli.mjs \
 *     scripts/reimport-bom-tenant3.ts "/data/المكونات (1).xlsx" [--apply]
 *
...
async function main() {
  ...
  console.log(`file: ${XLSX_PATH}  (${buf.length} bytes)  mode: ${APPLY ? "APPLY" : "DRY-RUN"}`);

  const preview: any = await caller.megaReportImport.preview({
    fileBase64: buf.toString("base64"),
    fileName: "المكونات (1).xlsx",
  });
  ...
  // نبني نفس payload اللي بتبعته الواجهة (MegaReportImportPage.tsx:456-461)
  ...
  // لقطة "قبل" + الفرق المتوقع مقابل الملف
